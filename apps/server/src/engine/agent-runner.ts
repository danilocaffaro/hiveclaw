...
Read their responses before duplicating work. Use @mentions to delegate or request help.`;
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    // Uses _iter tag on tool messages for accurate age tracking instead of
    // index estimation (which breaks with variable tool calls per iteration).
    const PRUNE_AGE_ITERATIONS = 3;
    const PRUNE_MIN_CHARS = 1000;
    const PRUNE_HEAD_CHARS = 200;
    const PRUNE_TAIL_CHARS = 200;

    // Tag this iteration's tool results for accurate pruning
    for (const msg of toolResultMessages) {
      (msg as unknown as Record<string, unknown>)._iter = iteration;
    }
